...
namespace ShowProducts.API.Controllers
{
    ...
    public class LoginController : ApiController
    {
        ...
        [Route("Authetication")]
        public async Task<IHttpActionResult> GetAsync(string user, string pas, bool? viewpage, string post)
        {
            ...
            if (result != null)
            {
                ...
                else
                {
                    ...
                    var user_model = db.Provider.FirstOrDefault(fs => fs.LoginID == login.ID);
                    var user_key_cookie = new CookieHeaderValue("user_key", user_model.ValidKey) { Expires = DateTime.Now.AddDays(30), Secure = true, HttpOnly = true, Domain = Request.RequestUri.Host, Path = "/" };
        ...
        {
            ShowProductsEntities db = new ShowProductsEntities();
            ...
            Provider user = logToken.Login1.Provider.FirstOrDefault(fs => true);

            //Valida a chave de verificação da tabela User.
            if (user.ValidKey != user_valid_key)
            {
                return new LoginInformations(user_valid_key, token) { IsValid = false, Message = "This 'login token' does not validing for your IP." };
            }
